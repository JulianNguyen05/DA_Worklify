...
**Worklify** là một nền tảng tuyển dụng thế hệ mới, được xây dựng với mục tiêu kết nối **ứng viên tài năng** và **nhà tuyển dụng uy tín** một cách hiệu quả và thông minh nhất. Không chỉ là một trang đăng tin tuyển dụng thông thường, Worklify tích hợp **mô hình AI/ML** để phân tích CV tự động và đề xuất mức độ phù hợp (matching score) giữa ứng viên và vị trí công việc — giúp rút ngắn đáng kể thời gian tuyển dụng cho cả hai phía.

### ✨ Điểm Nổi Bật

- 🤖 **AI-Powered Matching** — Tự động phân tích CV (PDF) bằng mô hình NLP/ML (FastAPI) và tính điểm tương thích với Job Description theo thời gian thực.
...
- ⚡ **Event-Driven** — Sử dụng **RabbitMQ** để điều phối sự kiện bất đồng bộ giữa backend core (Java) và backend ML (Python), tránh tắc nghẽn khi xử lý hàng loạt hồ sơ.
- 🔐 **Bảo Mật Nhiều Lớp** — Xác thực bằng JWT, phân quyền rõ ràng cho 3 vai trò: `CANDIDATE`, `EMPLOYER`, `ADMIN`.
- 📊 **Quản Trị Toàn Diện** — Hệ thống Admin với đầy đủ công cụ kiểm duyệt nội dung, quản lý người dùng và xuất báo cáo.
...
<table>
  <tr>
    <td align="center" width="50%">
      ...
      <br/><sub>Tổng quan cá nhân: số lượng hồ sơ đã nộp, trạng thái, việc làm được AI đề xuất phù hợp với hồ sơ.</sub>
    </td>
    <td align="center" width="50%">
      ...
      <br/><sub>Công cụ xây dựng và tải lên CV định dạng PDF. CV được AI phân tích để trích xuất kỹ năng và kinh nghiệm tự động.</sub>
    </td>
  </tr>
  <tr>
    <td align="center" width="50%">
...
<table>
  ...
  <tr>
    <td align="center" width="50%">
      ...
      <br/><sub>Xem tất cả hồ sơ ứng tuyển cho từng vị trí, sắp xếp theo điểm AI matching và thời gian nộp.</sub>
    </td>
    <td align="center" width="50%">
      ...
      <br/><sub>Xem CV ứng viên, điểm matching AI, thông tin cá nhân và cập nhật trạng thái xử lý hồ sơ.</sub>
    </td>
  </tr>
  <tr>
    <td align="center" width="50%">
...
| Trang | Mô tả chức năng |
|---|---|
| **Dashboard** | Thống kê nhanh: hồ sơ đang xét duyệt, tỉ lệ phản hồi, danh sách việc làm AI gợi ý dựa trên CV và kỹ năng đã cập nhật. |
| **CV Builder** | Upload file PDF (CV), xem trước trực tiếp. Hệ thống gửi file qua RabbitMQ → Backend ML phân tích và trích xuất thông tin cấu trúc (kỹ năng, kinh nghiệm, học vấn). |
| **My Applications** | Danh sách tất cả hồ sơ đã nộp với trạng thái từng bước trong quy trình tuyển dụng. Hiển thị điểm AI matching tương ứng với từng Job. |
...
| **Application List** | Xem toàn bộ hồ sơ cho một vị trí cụ thể. Hiển thị **AI Matching Score** nổi bật, bộ lọc theo trạng thái và sắp xếp theo điểm số. |
| **Application Detail** | Chi tiết hồ sơ ứng viên: xem CV (PDF viewer), thông tin cá nhân, kỹ năng trích xuất bởi AI và khả năng cập nhật trạng thái xử lý. |
...
| **Category Management** | CRUD đầy đủ danh mục ngành nghề (Industry) và kỹ năng (Skill) — dữ liệu nền tảng phục vụ toàn bộ hệ thống lọc và matching. |
...
- **Messaging** — `DomainEventRabbitMqDispatcher` lắng nghe Domain Event và điều phối sang RabbitMQ queue để Backend ML (Python/FastAPI) tiêu thụ và xử lý bất đồng bộ.
...
> **Package:** `backend-ml/app`

Một microservice độc lập, giao tiếp với backend core qua **RabbitMQ**:

- **`/parser`** — Nhận file CV (PDF), trích xuất thông tin cấu trúc (tên, kỹ năng, kinh nghiệm) bằng NLP.
- **`/matcher`** — Nhận dữ liệu CV đã parse và Job Description, tính toán **điểm tương thích (Matching Score)** bằng mô hình ML (cosine similarity, BERT embeddings...).
- Kết quả được ghi trả ngược về database qua REST call hoặc message queue.
...
### Bước 3 — Cấu Hình Backend ML (Python / FastAPI)
...
Cấu hình biến môi trường trong file `backend-ml/app/core/config.py` hoặc tạo file `.env`:
...
API_SECRET_KEY=your-ml-api-secret-key
```
...
✅ Backend ML sẽ khởi động tại `http://localhost:8000`
...
Để khởi động toàn bộ hệ thống (MySQL + RabbitMQ + Backend Core + Backend ML + Frontend) chỉ với một lệnh:
...
| 📚 Swagger UI | `http://localhost:8080/swagger-ui/index.html` |
| 🤖 Backend ML API | `http://localhost:8000` |
...
│   ├── 📁 infrastructure/   #   → Layer 3: JPA, Security, RabbitMQ, File Storage
│   └── 📁 api/              #   → Layer 4: REST Controllers, Exception Handler
│
├── 📁 backend-ml/           # Python FastAPI — CV Parser & Job Matcher
│   └── 📁 app/
│       ├── 📁 api/          #   → Endpoints: /parser, /matcher
...
**Được xây dựng với ❤️ bởi Team Worklify**
