...
import { MapPin, Globe, Briefcase, ExternalLink, Loader2, Building2 } from 'lucide-react';
...
export default function CompanyDetailPage() {
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      ...
      try {
        ...
        try {
          const jobsResponse = await employerService.getMyJobs(id, 0, 10);
          const jobsData = jobsResponse.data?.items || jobsResponse.data?.content || jobsResponse.data || [];
        ...
        setError("Không thể tải thông tin công ty. Có thể công ty này không tồn tại.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-blue-600 gap-4">
        ...
        <p className="text-gray-500 font-medium text-lg">Đang tải hồ sơ doanh nghiệp...</p>
      </div>
    );
  }
  ...
  if (error || !company) {
    return (
      <div className="text-center py-20 min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <p className="text-gray-500 text-xl font-medium">{error || "Không tìm thấy doanh nghiệp này!"}</p>
        <Link to="/companies" className="mt-6 text-blue-600 hover:underline font-medium">
  ...
  const defaultBanner = "https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2000&auto=format&fit=crop";
          ...
          <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="prose prose-blue max-w-none">
              {company.description ? (
                ...
                <p className="text-gray-400 italic">Công ty này chưa cập nhật bài giới thiệu.</p>
              )}
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-1 space-y-8">
          <section className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 sticky top-24">
            <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-3">
              ...
              Việc làm đang tuyển <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-sm">{activeJobs.length}</span>
            </h2>
            
            {activeJobs.length === 0 ? (
              <div className="bg-slate-50 p-6 rounded-2xl text-center border border-slate-100">
                <p className="text-gray-500 font-medium">Doanh nghiệp này hiện chưa có tin tuyển dụng nào.</p>
              </div>
            ) : (
