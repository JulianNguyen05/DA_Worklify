  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [cvRes, appsRes, jobsRes, companiesRes] =
        ...
        const sortedCompanies = (companies?.content || []).sort(
        ...
        setUiState((prev) => ({
          ...prev,
          error: "Đã xảy ra lỗi khi đồng bộ dữ liệu hệ thống.",
        }));
      } finally {
          ...
          <div className="divide-y divide-gray-100">
            {data.latestJobs.length > 0 ? (
              data.latestJobs.map((job) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-500 mt-1 line-clamp-1">
                    {job.companyName || "Công ty bảo mật thông tin"}
                  </p>
                  <div className="mt-3 flex items-center gap-3 text-xs font-medium">
                    <span className="bg-green-100 text-green-700 px-2.5 py-1 rounded-md">
              ...
              <div className="px-6 py-8 text-center text-sm text-gray-500">
                Chưa có công việc nào được cập nhật.
              </div>
            )}
          </div>
        </div>
...
const StatCard = ({ title, value, link, linkText, icon, colorClass }) => (
  <div
    className={`p-6 rounded-2xl border flex flex-col justify-between transition-all hover:shadow-lg ${colorClass}`}
  >
    ...
    <div className="mt-6">
      <Link
        to={link}
        className="inline-flex items-center text-sm font-semibold text-gray-700 hover:text-black bg-white/50 hover:bg-white px-3 py-1.5 rounded-lg transition-all border border-transparent hover:border-gray-200"
      >
