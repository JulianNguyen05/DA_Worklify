...
export default function CompanyModerationPage() {
  ...
  const fetchPendingCompanies = async () => {
    ...
    try {
      ...
      setStatusMsg({
        type: "error",
        message: "Lỗi tải danh sách doanh nghiệp.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleApprove = async (companyId) => {
    try {
      ...
      setStatusMsg({
        type: "success",
        message: "Đã duyệt hồ sơ doanh nghiệp thành công.",
      });
  ...
  const handleConfirmReject = async () => {
    if (!rejectReason.trim()) {
      setStatusMsg({
        type: "error",
        message: "Vui lòng nhập lý do từ chối để thông báo cho doanh nghiệp.",
      });
      return;
    }

    try {
      await adminService.moderateCompany(
        selectedCompany.id,
        "REJECTED",
        rejectReason,
      );
      setStatusMsg({
        type: "success",
        message: "Đã từ chối hồ sơ doanh nghiệp.",
      });
      ...
      render: (row) => (
        <div className="flex justify-center">
          <Button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-sm border-gray-300 text-gray-700 hover:bg-gray-50 hover:text-blue-600 hover:border-blue-300 transition-all"
          >
    ...
    <div className="max-w-6xl mx-auto pb-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm">
          Quản lý và phê duyệt hồ sơ đăng ký từ các nhà tuyển dụng mới trên hệ
          thống.
        </p>
      </div>
        ...
        <Table
          ...
          emptyMessage={
            <div className="py-12 flex flex-col items-center justify-center text-gray-500">
              ...
              <p className="text-sm">
                Tất cả hồ sơ doanh nghiệp đã được xử lý.
              </p>
            </div>
          }
        />
            ...
            <div className="flex items-start gap-5">
              <div className="w-20 h-20 bg-gray-50 border border-gray-200 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 shadow-sm">
                {selectedCompany.logoUrl ? (
                  <img
                    src={`http://localhost:8080${selectedCompany.logoUrl}`}
                    alt="Logo"
      ...
      >
        <div className="flex flex-col gap-5">
          <div className="bg-red-50 text-red-700 p-3 rounded-lg flex gap-3 text-sm border border-red-100">
            ...
            <p>
              Hành động này sẽ gửi thông báo yêu cầu doanh nghiệp{" "}
              <strong>{selectedCompany?.companyName}</strong> cập nhật lại thông
              tin. Vui lòng ghi rõ lý do.
            </p>
          </div>
