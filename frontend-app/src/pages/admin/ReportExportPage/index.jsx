...
export default function ReportExportPage() {
  ...
  const handleExport = async (format) => {
    if (!formData.startDate || !formData.endDate) {
      setStatusMsg({
        type: "error",
        message: "Vui lòng chọn đầy đủ khoảng thời gian.",
      });
      return;
    }
    ...
    try {
      ...
      setStatusMsg({
        type: "success",
        message: `Đã xuất báo cáo dạng ${format.toUpperCase()} thành công!`,
      });
    } catch (error) {
      setStatusMsg({
        type: "error",
        message: "Có lỗi xảy ra trong quá trình kết xuất.",
      });
    } finally {
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 max-w-2xl">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-1">
          ...
          >
            <option value="USERS">
              Tăng trưởng Người dùng (Ứng viên & Nhà tuyển dụng)
            </option>
            <option value="JOBS">Thống kê Tin tuyển dụng</option>
            <option value="APPLICATIONS">
              Hiệu suất Ứng tuyển & Tỷ lệ chuyển đổi
            </option>
          </select>
        </div>
